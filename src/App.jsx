 ...
 * Features:
 * - Partitioned Data Loading (Fast & GitHub Friendly)
 * - Drag-to-Pan & Zoom navigation
 * - Standardized Charts (Global Day Max)
 * - Metadata & Behavioral Analysis Panel (Bottom-Right)
 * - Context-Aware Legends (Percentage & Status)
 */
function App() {
    // 1. State Management
    ...
    const SUBWAY_LINES = [
        ...
        { "line": "5호선", "color": "#996CAC", "stations": ["방화", "김포공항", "송정", "마곡", "발산", "우장산", "화곡", "까치산", "신정", "목동", "오목교", "양평", "영등포구청", "영등포시장", "신길", "여의도", "여의나루", "마포", "공덕", "애오개", "충정로", "서대문", "광화문", "종로3가", "을지로4가", "동대문역사문화공원", "청구", "신금호", "행당", "왕십리", "마장", "답십리", "장한평", "군자", "아차산", "광나루", "천호", "강동"] },
        ...
        { "line": "7호선", "color": "#747F00", "stations": ["장암", "도봉산", "수락산", "마들", "노원", "중계", "하계", "공릉", "태릉입구", "먹골", "중화", "상봉", "면목", "사가정", "용마산", "중곡", "군자", "어린이대공원", "건대입구", "뚝섬유원지", "청담", "강남구청", "학동", "논현", "반포", "고속터미널", "내방", "이수", "남성", "숭실대입구", "상도", "장승배기", "신대방삼거리", "보라매", "신풍", "대림", "남구로", "가산디지털단지", "철산", "광명사거리"] },
    ...
    useEffect(() => {
        ...
        fetch(`daily_data/${selectedDate}.json`)
            ...
            .catch(err => console.error("Daily Data Error:", err));
    }, [selectedDate]);
    ...
    const maxValAtTime = useMemo(() => {
        ...
        stations.forEach(s => {
            ...
            if (!isVisible) return;
            let val = (viewMode === 'congestion') 
                ? (s.hourly_congestion[currentTime] || 0)
                : Math.abs((s.hourly_inflow[currentTime] || 0) - (s.hourly_outflow[currentTime] || 0));
            if (val > max) max = val;
        });
        return max || 1;
    ...
    const getStationStyle = (station) => {
        const isTargetLine = selectedLine === 'All' || station.line === selectedLine || station.line.startsWith(selectedLine);
        let radius = 0.2; let color = "#2ecc71";
        const effMax = maxValAtTime > 0 ? maxValAtTime : 1;

        if (viewMode === 'congestion') {
            const ratio = Math.min(1.5, (station.hourly_congestion[currentTime] || 0) / effMax);
            ...
            const ratio = Math.min(1.5, Math.abs(diff) / effMax);
        ...
        return { radius, color, opacity: isTargetLine ? 1 : 0.05, interactive: isTargetLine };
    };
    ...
    const getBehavior = (s) => {
        ...
        return {
            morning: m > 0 ? "Inflow" : "Outflow",
            noon: n > 0 ? "Inflow" : "Outflow",
            evening: e > 0 ? "Inflow" : "Outflow"
        };
    };

    const formatNum = (n) => new Intl.NumberFormat().format(Math.round(n));
    const dayInfo = currentDay || { weather: { temp: 0, condition: 'Clear' }, events: [] };
                        ...
                        {selectedStation ? (
                            <div className="dashboard-content">
                                ...
                                <div className="chart-container">
                                    ...
                                    <svg viewBox="0 0 350 140" className="chart-svg">
                                        {(() => {
                                            ...
                                            return (
                                                <>
                                                    <line x1="40" y1="30" x2="40" y2="110" stroke="#bdc3c7" strokeWidth="1" />
                                                    <line x1="40" y1="110" x2="330" y2="110" stroke="#bdc3c7" strokeWidth="1" />
                                                    ...
                                                    <text x={getX(currentTime)} y={getY(data[currentTime]) - 8} fill="#e74c3c" fontSize="11" fontWeight="bold" textAnchor="middle">{formatNum(data[currentTime])}</text>
                                                </>
                                            );
                                        })()}
                                    </svg>
                                    ...
                                    <svg viewBox="0 0 350 140" className="chart-svg">
                                        {(() => {
                                            ...
                                            return (
                                                <>
                                                    <line x1="40" y1="30" x2="40" y2="110" stroke="#bdc3c7" strokeWidth="1" />
                                                    <line x1="40" y1="70" x2="330" y2="70" stroke="#bdc3c7" strokeWidth="1" strokeDasharray="4" />
                                                    <text x="35" y={getY(maxAbsVal) + 4} fontSize="9" fill="#7f8c8d" textAnchor="end">+{formatNum(maxAbsVal)}</text>
                                                    <text x="35" y={getY(-maxAbsVal) + 4} fontSize="9" fill="#7f8c8d" textAnchor="end">-{formatNum(maxAbsVal)}</text>
                                                    {[0, 6, 12, 18, 23].map(h => (<text key={h} x={getX(h)} y="125" fontSize="10" fill="#7f8c8d" textAnchor="middle">{h}h</text>))}
                                                    <polyline points={data.map((v, i) => `${getX(i)},${getY(v)}`).join(' ')} fill="none" stroke="#2980b9" strokeWidth="2" />
                                                    <line x1={getX(currentTime)} y1="30" x2={getX(currentTime)} y2="110" stroke="#e74c3c" strokeDasharray="3" />
                                                    <circle cx={getX(currentTime)} cy={getY(data[currentTime])} r="3" fill="#e74c3c" />
                                                    <text x={getX(currentTime)} y={getY(data[currentTime]) - 8} fill="#e74c3c" fontSize="11" fontWeight="bold" textAnchor="middle">{data[currentTime] > 0 ? '+' : ''}{formatNum(data[currentTime])}</text>
                                                </>
                                            );
                                        })()}
                                    </svg>
                                ...
                                <div className="chart-container">
                                    <h4>Hourly Population Histogram</h4>
                                    <svg viewBox="0 0 350 140" className="chart-svg">
                                        {(() => {
                                            const data = selectedStation.hourly_congestion; const max = globalDailyMaxCongestion; 
                                            const getX = i => 40 + i * (290 / 23); const getY = v => 110 - (v / max) * 80;
                                            return (
                                                <>
                                                    ...
                                                    <line x1="40" y1="110" x2="330" y2="110" stroke="#bdc3c7" strokeWidth="1" />
                                                    {data.map((val, idx) => (
                                                        <rect key={idx} x={getX(idx) - 3} y={getY(val)} width="6" height={110 - getY(val)} fill={idx === currentTime ? "#e74c3c" : "#bdc3c7"} />
                                                    ))}
                                                    {[0, 6, 12, 18, 23].map(h => (<text key={h} x={getX(h)} y="125" fontSize="10" fill="#7f8c8d" textAnchor="middle">{h}h</text>))}
                                                    <text x={getX(currentTime)} y={getY(data[currentTime]) - 5} fill="#e74c3c" fontSize="10" fontWeight="bold" textAnchor="middle">{formatNum(data[currentTime])}</text>
                                                </>
                                            );
                                        })()}
                                    </svg>
                                ...
                                <div className="similar-stations-container">
                                    ...
                                    <ul className="similar-list">
                                        {selectedStation.similar_stations?.map((sim, i) => {
                                            const sData = stations.find(st => st.name === sim.name);
                                            return <li key={i}><span>{sim.name} <small style={{color:'#7f8c8d'}}>({sData?.line || 'Other'})</small></span><strong>{sim.score}%</strong></li>;
                                        })}
                                    </ul>
                                </div>
                            </div>
